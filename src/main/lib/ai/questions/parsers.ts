...
export const parseAIQuestionResponse = (input: string): AIQuestionResponse => {
    ...
    if (!learnMore) {
      log.error('ai', `Failed to parse learnMore this is what I tried to parse: ${input}`);
    }
  
    if (!distractors.length || distractors.length !== 3) {
      log.error('ai', `Failed to parse distractors this is what I tried to parse: ${input}`);
    }
