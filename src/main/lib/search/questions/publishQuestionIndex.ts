...
export const publishQuestionIndex = async (items: AlgoliaRecord[]) => {
  ...
  const requests = items.map((item) => {
    ...
    const record = typeof item.record === 'object' && item.record !== null ? item.record : {};
  ...
  try {
    const response = await axios.post(
      `${baseUrl}/1/indexes/questions/batch`, // Use the /batch endpoint
      { requests },
      {
        headers: headers,
      },
...
export default publishQuestionIndex;
