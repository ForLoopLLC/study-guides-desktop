...
export const unpublishQuestionIndex = async (id: string) => {
  const headers = environmentManager.getAlgoliaHeaders();
  const baseUrl = environmentManager.getAlgoliaBaseUrl();

  try {
    const response = await axios.post(
      `${baseUrl}/1/indexes/questions/batch`, // Use the /batch endpoint for deletion
      {
        requests: [
          {
...
export default unpublishQuestionIndex;
